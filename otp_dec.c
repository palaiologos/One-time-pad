...
// Slightly modified to send cipher text instead of 
// plain text. For ease of copying purposes, cipher
// text is referred to as plain text in this, just like
// otp_enc.c
...
int main(int argc, char *argv[])
{
  ...
  for (i = 0; i < length - 2; i++)
  {
    ...
    if (plain_text_buffer[i] > 90 || ( plain_text_buffer[i] < 65 && plain_text_buffer[i] != 32) )
    {
      ...
      fprintf(stderr, "Bad characters found in file\n");
      exit(1);
    }
  ...
  strncpy(key_truncate, key_buffer, length );
  key_buffer_length = (length );
  ...
  */
  ...
  //printf("CLIENT: decoded text received: %s\n", cipher_text);
  // This is actually the decoded message, I just didn't change the variable names after
  // I copied the program from otp_enc to save time.
