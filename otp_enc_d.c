...
int main(int argc, char *argv[])
{
  ...
  while(1)
  {
    // Was going to try select() but ran out of time.
    ...
    if (spawn_pid == 0)
    {
      ...
      //printf("SERVER: Done sending and receing info from client\n");
      ...
      //printf("SERVER: about to encrypt\n");
	...
	if (plain_text_buffer[i] == ' ')
        ...
        // Else, get its position by subtracting 65 from its ascii value, since A = 65.
        // That makes A become the 0 for our character list defined earlier. It basically just
        // shifts them all down 65 so each uppercase char fits into our scheme here.
      ...
      //printf("SERVER: cipher text sent: %s\n", cipher_text);

      // After encryption, send ciphertext back to client.
      msg_sent = send(establishedConnectionFD, cipher_text, sizeof(cipher_text), 0);
      if (msg_sent < 0)
      {
        fprintf(stderr, "SERVER: failed to send cipher text to client\n");
        exit(1);
      } 

      // Finish and go to parent.
      //printf("CHILD: finished everything and closing socket and exiting\n");
      exit(0);
    }
    // Otherwise, close and we are in the parent. Close socket and set to bad value.
    else
    {
      // Wait for child to finish, then continue on.
      //printf("Waiting for child to terminate\n");
      waitpid(spawn_pid, &childExitMethod, 0);
      //printf("In parent. Child is done now. Closing socket FD and relooping\n");
    }
    // Close it per multiserver.c
